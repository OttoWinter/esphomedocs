...
- **shunt_resistance** (*Optional*, float): The value of the shunt resistor on the board for current calculation.
  Defaults to ``0.1 ohm``. **Note:** Some INA219 breakout boards that have a ``0.1 ohm`` shunt will give values 
  of current that is off by a factor of 100. A workaround is to set shunt_resistance to ``0.001 ohm``.
