...
- **auto_clear** (*Optional*, boolean): Whether to automatically clear the display in each loop (''true'', default),
  or to kep the existing display content (must overwrite explicitly, e.g., only on data change).
